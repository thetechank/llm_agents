...
booking_agent = Agent(
    ...
    model="gpt-4o-mini",
    instructions="""
    You are a polite booking assistant. When handed off, reply back with Postive booking confirmation for the city with price and weather
    """,
    handoff_description="You are a agent responsible for booking the flight. Reply with all details of booking such as City, Price and also the expected weather"
)
...
async def chat_fn(message, history):
    ...
    with trace(trace_name, trace_id = make_trace_id(trace_name), group_id="Flight1"):
        try:
            ...
            result = await runner.run(main_agent, items +  [{"role": "user", "content": message}]) # type: ignore[attr-defined]
        ...
        # If a handoff occurred, print it clearly
        # if result.handoff_target: # type: ignore[attr-defined]
        #     print(f"🔁 Handoff detected to agent: {result.handoff_target.name}")
        #     if hasattr(result, "handoff_result") and result.handoff_result:
        #         print(f"📨 {result.handoff_target.name} replied: {result.handoff_result.final_output}")

        # print(f"📨 Main Agent: {result.final_output}")
        # return result.final_output
        # target = getattr(result, "handoff_target", None)
        # if target:
        #     print(f"🔁 Handoff detected to {target.name}")
        #     sub = getattr(result, "handoff_result", None)
        #     if sub:
        #         print(f"📨 {target.name}: {sub.final_output}")
        # else:
        #     print(f"📨 Main Agent: {result.final_output}")
